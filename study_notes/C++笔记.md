...
我觉得我十分有必要说一下文件组织规范的问题了。

对于大规模的程序，正如在单个程序文件中，我们会把某些经常使用的常量define为常量，对于我们要在多个文件中使用的函数和类，我们也自然会把这些类或函数单独定义，对于这种情况，我们称之分离式编译。

呃，我并不太确定类或者结构体是否有声明，对于函数是有声明和定义之分的，对于这种情况，我们会将定义写在一个源文件中，然后将声明写在一个头文件中，然后在要是用这个函数的文件中引用头文件即可。对于类，我们可以直接将定义写在头文件中，其实函数也可以，只是为了更加清晰起见，会单独列出一个源文件。

那么接下来的问题可以分为两个部分，第一头文件中如何预防重复声明的问题，第二如何编译。

对于前者，这个问题的出现是很自然的，某些角度来说也是很常见的，为了解决这个问题，我们使用条件预编译指令，或者说头文件保护符功能，具体而言，策略就是我们设置一个特殊的flage，然后再头文件中最开始的部分检查这个flage是否已经设立，如果没有设立的话说明还没有进行第一次声明，那么我们将设立这个flage，然后允许预编译，如果已经设立，那么就不再进行预编译。
...
这里需要注意的一个问题就是，我们要保证这个flage不会在其他地方被设置或处理，方法就是让它尽可能特殊，所以策略就是全大写，然后插入一些下划线，在这里`STAN_H`就是flage，所以他只是一个特殊的flage而已，一个名字而已，没有任何特别的规则，准确的说我可以任意命名，只要符合命名规范就好。

第二个要考虑的问题是如何编译，事实上，因为我使用IDE，所以我并不需要考虑这个问题。
...
但是如果非要知道迭代器的类型，那么拥有迭代器的标准库容器都在自己的命名空间里定义了两个类型，分别是：`iterator和const_iterator`，二者的区别是后者只读，举个栗子：`string::iterator a;`

又如：`vector<int>::const_iterator b;`
...
我几乎从来没有用过switch，这里只提一点，switch的值或表达式必须是一个可以转化为整数的



#### 异常处理

throw语句可以抛出错误，例如`throw runtime_error("it is wrong")`

平常使用的是一系列的try...catch语句
...
catch的异常有很多，最基本最普通的是定义在exception头文件中的exception
...
到今晚10点半还有3个小时，我应该可以解决函数这一部分。

函数大概还是比较清楚的，对吧，但是事实上函数部分还是比较复杂的

首先，函数的返回值不能是任意类型，不能是数组，也不能是函数，但是可以是指向二者的指针

#### 声明与分离式编译

我并不习惯于使用声明，我也并无法感受到分离式编译的好处，所以这一部分暂时略过



#### 参数

我错了，我之前有说过，对于所有高级语言而言，只要你给函数传入的是构造类型，那么必然是传引用，不用返回，改变自然会同步到原变量中。

记忆中，我对于c和python的测试都是如此。现在c++证明我错了。

准确的说，函数的参数的情况的确是分为传值和传引用，基本上只要形参不是引用类型，那么都是传值，也就意味着只是传入了一份拷贝而已，绝对不会影响到外部。我测试了字符串和结构体，的确如此。

但是，有一个例外，数组，因为数组是不支持直接拷贝的，所以数组传递的实际上是指针，所以修改会直接被映射到原地址空间。
...
当我们想让函数可以接受不限数量的参数的时候，如果这些参数类型全部相同，可以使用`initializer_list`容器，如果类型不一致，那就是高级版了，后面会讲。

`initializer_list`定义在同名头文件中，使用方式颇类似于vector，它会收集一系列参数，函数内部应该使用迭代器进行访问。但是传入的这些参数必须放在{}中，它可以和其他类型同时存在于参数中。
...
void change(initializer_list<string> old,initializer_list<int> se)
{
    for(auto p=old.begin();p!=old.end();p++) cout<< *p <<endl;
    for(auto p1=se.begin();p1!=se.end();p1++) cout<< *p1 <<endl;
}

int main()
{

    change({"hello","this","is","stan"},{1,2,3,4});
...
其实极少用，只是在c++与c访问的时候才有用，暂时不说了。

#### 返回值

函数的局部变量会在完成之后就被释放，所以不要返回它们的引用或者指针

现在可以返回一系列的值，但是函数应该被定义为vector，然后返回值写在列表中
...
名字相同的函数具有不同的形参列表，称之函数重载

呃，深入的讲其实也有很多，但是感觉根本用不上，所以暂时就这样

#### 特殊用途

##### 缺省参数

我们应该为后面的形参设置缺省，不能是前面的，当你传入参数时，会自左向右开始匹配，我们不能跳过最左边的，也就是说，我们可以覆盖缺省参数，但是如果想覆盖一个缺省参数，就必须覆盖它前面所有的。

##### 内联函数

可以在函数的最前面加入inline修饰符，让其变为内联函数，但是这只是向编译器提供了一个标记，编译器想不想支持无法决定。
...
内容太多了，感觉搞不定了，算了，函数就暂时跳过吧，有需求再回头来过，拜拜，不想学了。



### 类

类的方法可以定义在类的外面，此时，为了表明这是类的方法，需要以类似于声明命名空间的方法指定：

~~~c++
int classname::funcname()
{
  
}
~~~

然后就是构造函数，构造函数没有返回值，可以重载。

自然是存在默认的构造函数的，如果我们想在自定义的的构造函数之外，依旧保留默认的构造函数，只需写`classname() = default;`即可



#### 构造函数初始值列表

一般而言，我们对构造函数所做的所有操作都应该写在函数体内，这个很自然，很常规。但是考虑一些特殊情况，我们为类定义了一些引用或常量属性，一般而言常量和应用必须在声明时就初始化，但是我们的确想让他们被初始化为构造函数的输入参数，这似乎是不可能的，于是出现了构造函数初始值列表，这种特殊的语法可以满足这种需求。
...
Stan::Stan(int value): i(value), j(value), ri(i)
{}
~~~

需要小心的点是，某些属性的初始值是依赖初始化顺序的，例如我们想先初始化i，然后让常量初始化为i，那么这时顺序就是一个需要考虑的因素，要注意的是构造函数初始值列表中写的顺序并不决定初始化顺序，起决定作用的是声明的顺序，例如i先声明，是类的第一个成员，j是第二个成员，那么构造函数初始值列表中一定是i先初始化，然后是j
...
我还是没什么进展，太多的东西学的不细致，渐渐的甚至连简单的代码都到处被掣肘。换个思路写一下笔记，只记录重要的，会迷惑我的问题。



### 自加

a++与++a的区别，如果只是为了实现a的自增，那么我认为并没有什么区别。只有当进行赋值操作的时候才有区别，可是认为这是一个表达式，这个表达式完成了一个操作，同时产生了一个返回值，完成的操作就是对a的自增，返回值也是a，问题就在于两者的顺序，可以直接观察++与a出现的顺序就好。所以a++是先返回了a，然后对a自增。至于++a自然是先完成++再返回a



### 类的实例化与组合类

什么叫做组合类？只要类的某个属性是一个其它类的实例，那么这个类就是组合类。组合类在初始化的时候涉及到了初始化列表。

换句话说，只要是组合类就要考虑初始化列表的问题。因为这涉及到属性的初始化顺序，属性的初始化是在类的初始化函数之前的，如果不用初始化列表初始化类型为自定义类的属性，那么系统将会自动调用这个属性的无参数构造函数，所以如果这个类你甚至没提供无参数构造函数甚至会直接报错。

其实也不尽然，我们可以在属性定义之初就直接将其初始化，这也是完全可以的。



所以可以看得出来，你要是不在意属性调用了哪个构造函数，那初始化列表就无所谓了，反之，必须使用初始化列表。

说回实例化，对于类的实例化，如果暂时不提及new的初始化方式，我认为常见的实例化方法有下面几种：
...
第一种只适用于这个类具有无参数构造函数的情形

第二种适用于有参数的构造函数，所以注意不能这样写`Stan a();`
...
这里其实要说的其实就是this的使用，this的唯一作用就是在出现变量名字与属性一致的时候作区分之用。

在定义类的方法的时候，我们可以直接使用属性的名字做引用，也可以加点东西，例如`Stan::a`，或者使用this，并且完全不用担心类与对象的问题，我们修改的绝对是对象的属性。



### 类的访问权限

一直以来，其实我并不在意所谓的protected,private,public。建议以后还是规范一点。
...
假设需要跨文件使用变量，不做任何操作是不可能实现的，需要使用extern，同时需要区分什么叫做变量的定义和声明。
...
跨文件使用变量，只能在一份文件内定义变量，在其他文件内只能声明变量。

据我测试，定义的位置不能位于头文件中，否则就会出现重复定义的警告。

定义的位置直接定义就好。

#### auto

默认情况下，局部变量的存储类型都是auto

#### register

当需要高速访问某个局部变量的时候，可以把它设置为register类型，存储于寄存器中，快，但是自然有限制，长度最大是寄存器的长度，不能取址。

最重要的是，这只是可能，具体的会不会存入寄存器还是有编译器，硬件等决定
...
只用来修饰类属性，例如类的mutable属性可以被const成员函数修改。
