...
- 如何隐藏文件和文件夹，同时甚至无法搜索到

~~~powershell
attrib +h +s * /s /d
~~~

这个命令的作用是设置当前文件夹下文件的属性，其中的`+`代表添加属性，`h, s`分别代表隐藏属性和系统属性，`*`是通配符，通配操作对象，`/s`代表处理所有满足条件的文件， `/d`代表处理所有满足条件的文件夹

重新显示的方法就是`attrib -h -s * /s /d`
...
方法1还有一个缺陷是如果电脑开机之后不登陆的话是不会执行脚本的。
