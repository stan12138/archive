...
python可以接受位置参数和关键字参数，那么怎么接受所有的这些参数，也就是说怎么让一个函数可以接受所有可能输入？
...
传参只有上述两种方式,args会自动接收所有位置参数,放入一个元组,kwargs会自动接收所有关键字参数,放入一个字典,所以以上函数,你应该会看到输出了一个元组和一个字典

如果我们要直接使用这些参数,自然直接在元组或者字典中索引就好了.

那么,假设我们想继续传递下去呢,我们还想把这些参数全部传递给另外一个函数呢?方法是解包,解包可以将这个元组和字典重新还原为位置参数和关键字参数的形式.

~~~python
def func(*args, **kwargs) :
    print("I get :", args, kwargs)
    func2(*args, **kwargs)
...
一个我们自定义的类,在实例化过程中经历了什么?

据我目前所知,首先会调用类的`__new__`方法,这个方法会接收到的第一个参数是这个类, 例如`<class '__main__.Meta'>`, 如果同时在实例化过程中提供了任何位置或者关键字参数,这些参数也会同时传递给这个方法.这个方法负责生成一个实例,生成的方式就是`return object.__new__(cls)`,这里的cls即第一个参数,所以,一个最简单的自定义`__new__`方法如下:
...
之后,`__init__`方法负责给这个新生成的实例设置各种属性,它也会接收到上述位置和关键字参数,同时刚才新生成的实例将自动作为第一个参数传递给它.
